"""
Gido's VAE without conv layers
"""
import numpy as np
import math
import torch
...
from models.class_vae.gido_class_vae.models.utils import loss_functions as lf, modules
from models.class_vae.gido_class_vae.models.fc.nets import MLP
from models.class_vae.gido_class_vae.models.fc.layers import fc_layer_split, fc_layer
from models.class_vae.gido_class_vae.utils import get_data_loader
...
class FFAutoEncoder(nn.Module):
    ...
    def __init__(self,
                 # -fc-layers
                 input_size=None,
                 fc_layers=3, fc_units=1000, h_dim=400, fc_drop=0,
                 fc_bn=False, fc_nl="relu",
                 # -prior
                 z_dim=20, prior="standard", n_modes=1,
                 # -decoder
                 recon_loss='MSE', network_output="sigmoid",
                 ):
        ...
        ##>----Encoder (= q[z|x])----<##
        if fc_layers < 2:
            self.fc_layer_sizes = [input_size]  # --> this results in self.fcE = modules.Identity()
        elif fc_layers == 2:
            self.fc_layer_sizes = [input_size, h_dim]
        else:
            self.fc_layer_sizes = [input_size] + [int(x) for x in np.linspace(fc_units, h_dim, num=fc_layers - 1)]

        self.fcE = MLP(size_per_layer=self.fc_layer_sizes, drop=fc_drop, batch_norm=fc_bn, nl=fc_nl)
        self.toZ = fc_layer_split(h_dim, z_dim, nl_mean='none', nl_logvar='none')

        ##>----Decoder (= p[x|z])----<##
        # -> if 'gp' is used in forward pass, size of first/final hidden layer
        # differs between forward and backward pass
        self.fromZ = fc_layer(z_dim, h_dim, batch_norm=fc_bn, nl=fc_nl)
        self.fcD = MLP(
            size_per_layer=[x for x in reversed(self.fc_layer_sizes)],
            drop=fc_drop, batch_norm=fc_bn, nl=fc_nl,
            output='normal'  # self.network_output if self.depth==0 else 'normal',
        )
    ...
    def get_name(self):
        fcE_label = "{}_".format(self.fcE.name) if self.fc_layers > 1 else f"h{self.input_size}_"
        z_label = "z{}{}".format(self.z_dim,
                                 "" if self.prior == "standard" else "-{}{}".format(self.prior, self.n_modes))
        return "{}={}{}".format(self.label, fcE_label, z_label)
    ...
    def feature_extractor(self, images):
        ...
        return self.fcE(self.convE(images))
    ...
    def loss_function(self, x, x_recon, mu, z, logvar, batch_weights=None):
        ...
        variatL /= self.input_size  # -> divide by input-pixels
    ...
    def estimate_loglikelihood(self, dataset, S=5000, batch_size=128, max_n=None, feature_extractor=None):
        ...
        ll_per_datapoint = []  # store estimated log-likelihood for each datapoint
        for index, (x, _) in enumerate(data_loader):
            if max_n is not None and index >= max_n:
                break  # Break loop if max number of samples has been reached
            if feature_extractor is not None:
                x = x.to(self._device())
                with torch.no_grad():
                    x = feature_extractor(x)
